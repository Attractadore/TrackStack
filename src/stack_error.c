...
// \brief Check if an array is filled with poisong
//
...
#ifdef USE_HASH
hash_type stack_metadata_hash(Stack* stk) {
    assert(stk);
...
hash_type stack_data_hash(Stack* stk) {
    assert(stk);
...
void stack_dump_elem(FILE* file, void const* data_p, size_t offset, size_t sz) {
    assert(file);
    assert(data_p);
    assert(sz);
...
void stack_dump(Stack* stk, int line, char const* func_name, char const* file_name, char const* log_name) {
    ...
    for (size_t i = 0; i < stk->capacity; i++) {
        ...
        stack_dump_elem(log_file, stk->data, i * stk->elem_sz, stk->elem_sz);
    }
    fprintf(log_file, "\n");
}
...
void stack_verify(Stack* stk, int line, char const* func_name, char const* file_name) {
    ...
    if (stk->error <= STACK_OK || stk->error >= STACK_CORRUPTION_ERROR) {
        stk->error = STACK_CORRUPTION_ERROR;
    }
    ...
    // TODO: do something about the abort()
